...
# %% auto 0
__all__ = ['extract_umi_cb', 'extract_dictionary', 'parse_plate', 'SequencesCells', 'FastQ', 'concatenate_tables']
...
def parse_plate(path, plate_name):
    ...
    with gzip.open(path, "rt") as handle:
        for record in SeqIO.parse(handle, "fastq"):
            ...
            cb = plate_name + '_' + cb 
            sequence = str(record.seq)
            id_item = (sequence, cb, umi)
            ids.append(id_item)
    df = pd.DataFrame(ids, columns=['seq', 'cb','umi'])
    dedup_df = df.drop_duplicates().reset_index(drop=True)
    count_df = dedup_df.groupby(['seq', 'cb']).count()
    seqcell_table = count_df.unstack(fill_value=0)
    seqcell_table.columns = seqcell_table.columns.droplevel()
    return seqcell_table


# %% ../01_FastQ.ipynb 16
class SequencesCells():
    def __init__(self, df=None, plate=[]):
        self.table = df
